...
- 策略名：`weekly_pure_momentum_base0.00_ret15_th0.04`。
- 规则：每周最后一个交易日确认，15 日涨幅高于 4.00% 时科创50仓位 100%，否则 0% 并进入现金管理。
- 只做多、不做空、不加杠杆，仓位限制 0%-100%。
- 信号收盘后确认，下一交易日执行。
- 单次仓位变化成本：0.10%。
- 空仓现金年化：2.00%。

2026-06-07 报告中的全样本结果：

```text
样本区间：20200304 至 20260605
策略年化收益：15.02%
持有不动年化收益：4.65%
相对持有年化超额：10.37%
策略累计收益：139.93%
...
- 当前评分：4.84 / 10，中性区。
- 当前策略仓位：100.00%。
- 下一交易日目标仓位：0.00%，提示降仓到现金管理。

重要 caveat：

- 达标候选只有 1 个，样本期较短，结论没有红利低波长期样本那么稳。
- 若空仓现金年化按 0 计算，策略年化约 13.41%，低于 15%。
- 若单次仓位变化成本升到 0.20%，策略年化约 14.04%，低于 15%。
