...
- 评分 `<= 4.0`：目标仓位 100%。
- 否则，收盘价 `> MA24`：目标仓位 100%。
- 否则：目标仓位 0%，未投入 H30269 的资金进入现金管理。
...
```bash
cd /mnt/ssd01/stocks
scripts/analyze_kcb50_strategy.py
...
- 因科创50历史较短，评分使用短周期温度模型：60日价格位置、60日回撤、20日均线偏离、RSI14、10/15/20日涨幅、20日波动、20日相对沪深300和创业板。
- 每个指标使用截至当日可见的扩展分位，至少 20 个有效样本后出分，避免未来函数。

生产策略固定机制（2026-06-10 起）：

- 每日海选第一名随新数据漂移（2026-06-05 选中 `weekly_pure_momentum_ret15_th0.04` 提示清仓，2026-06-08 起换成 `monthly_band_base0.0_ma25_low3.0_band0.02` 提示满仓），仓位提示来回翻转，因此驱动仓位的生产策略改为固定在 `config/kcb50_production_strategy.json`。
- 海选网格每日照常计算，但只作研究参考；报告会同时显示生产策略和当日海选第一名。
- 生产策略不满足筛选目标时，报告和 stdout 输出 ⚠️ 告警，但不自动切换；换策略需人工修改配置文件并记录原因。
- 配置文件缺失或策略名不在候选集中时，退回自动海选并在报告中告警。

当前固定的生产策略：

- 策略名：`monthly_band_base0.0_ma25_low3.0_band0.02`。
- 规则：每月最后一个交易日确认；收盘价突破 MA25 上方 2% 或评分 <= 3.0 时满仓；跌破 MA25 下方 2% 且评分 > 3.0 时仓位 0% 进现金管理；其他情况保持上一目标。
- 只做多、不做空、不加杠杆，仓位限制 0%-100%；信号收盘后确认，下一交易日执行。
- 单次仓位变化成本：0.10%；空仓现金年化：2.00%。

2026-06-10 报告中的全样本结果（数据截至 2026-06-09）：
...
策略最大回撤：-27.76%
持有不动最大回撤：-62.7%
2022-2023 年化：-7.33%（唯一不达标项，报告持续告警）
2024 至今年化：+50.86%
年均换手：5.1 次
```
