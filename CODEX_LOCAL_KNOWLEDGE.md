...
但本机 `scripts/query_server.py` 的实际计算是：

```python
return pe / roe
```

代码注释说明 Tushare ROE 字段是百分点，例如 `10` 表示 `10%`。页面实测也符合 `PE / ROE百分点`：贵州茅台示例为 `19.16 / 10.57 = 1.8130`。后续如用户强调原始定义，需要先确认是否要修改代码口径；当前生产页面以代码口径为准。
